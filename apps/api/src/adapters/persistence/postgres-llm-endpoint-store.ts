...
const ROW = `id, label, base_url AS "baseUrl", model, key_cipher AS "keyCipher", created_at AS "createdAt"`;
...
export class PostgresLlmEndpointStore implements LlmEndpointStore {
  ...
  async create(rec: StoredLlmEndpoint): Promise<StoredLlmEndpoint> {
    await this.pool.query(
      `INSERT INTO llm_endpoints (id, label, base_url, model, key_cipher, created_at)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [rec.id, rec.label, rec.baseUrl, rec.model, rec.keyCipher, rec.createdAt]
    );
    return rec;
  }
