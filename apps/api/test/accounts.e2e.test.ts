...
describe("accounts (enterprise auth)", () => {
  ...
  describe("bearer auth + roles", () => {
    ...
    it("token idle window slides with activity and expires after silence", async () => {
      const token = await login("anh.nguyen", "test-admin-password");
      ...
      // Pin an explicit 30-min window so the mechanism is tested deterministically,
      // independent of the (now much larger) default idle TTL.
      ...
      expect(await store.findByTokenHash(hash, t29, IDLE)).not.toBeNull();
      // 29 + 29 minutes: still valid because activity refreshed the window
      const t58 = new Date(t29.getTime() + 29 * 60_000);
      expect(await store.findByTokenHash(hash, t58, IDLE)).not.toBeNull();
      // 31 minutes of silence: expired — and permanently revoked
      const t89 = new Date(t58.getTime() + 31 * 60_000);
      expect(await store.findByTokenHash(hash, t89, IDLE)).toBeNull();
      expect(await store.findByTokenHash(hash, t89, IDLE)).toBeNull();
    });

    it("default idle TTL is comfortably long for a desktop cockpit (≥ 1 day)", () => {
      expect(AccountsService.idleMs()).toBeGreaterThanOrEqual(24 * 60 * 60_000);
    });
