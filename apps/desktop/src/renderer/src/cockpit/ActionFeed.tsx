...
import { useRelay, RelayOverlay, RelayStyles, type RelayItem } from "./relay";
...
export default function ActionFeed({ active = true }: { sessionId?: string; active?: boolean }) {
  ...
  // The replay stream: Claude's actions (drop user prompts), newest last.
  const relayItems = useMemo<RelayItem[]>(
    () => acts.filter((a) => a.kind !== "prompt").map((a) => ({ key: a.key, label: a.label, icon: a.icon, color: META[a.kind].color, detail: a.detail })),
    [acts],
  );
  ...
  return (
    <div className="rcw-af">
      ...
      {relay.playing && <RelayOverlay queue={relay.queue} idx={relay.idx} title="Incoming transmission" onDismiss={relay.dismiss} />}
    </div>
  );
}
