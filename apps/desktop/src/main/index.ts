...
ipcMain.handle(IPC.jiraProfileProjects, (_e, a: { name: string }) => api.jiraProfileProjects(a.name));
ipcMain.handle(IPC.jiraProfileUsers, (_e, a: { name: string; q: string }) => api.jiraProfileUsers(a.name, a.q));
ipcMain.handle(IPC.agencyChatList, (_e, a: { afterId?: string }) => api.agencyChatList(a?.afterId));
ipcMain.handle(IPC.agencyChatPost, (_e, a: { body: string; attachments?: { name: string; url: string; size: number; mime: string }[] }) => api.agencyChatPost(a.body, a.attachments ?? []));
...
ipcMain.handle(IPC.agencyWeek, () => api.agencyWeek());
ipcMain.handle(IPC.agencyWeekConfig, (_e, a: { prize: string; startsAt: string | null; endsAt: string | null }) => api.agencyWeekConfig(a));
ipcMain.handle(IPC.scoringBoard, (_e, a: { project: string; week?: string }) => api.scoringBoard(a.project, a?.week));
ipcMain.handle(IPC.scoringMy, (_e, a: { project?: string }) => api.scoringMy(a?.project));
ipcMain.handle(IPC.scoringHistory, (_e, a: { project?: string }) => api.scoringHistory(a?.project));
ipcMain.handle(IPC.scoringProjects, () => api.scoringProjects());
ipcMain.handle(IPC.scoringAgentWeek, () => api.scoringAgentWeek());
ipcMain.handle(IPC.scoringConfigGet, (_e, a: { project: string }) => api.scoringConfigGet(a.project));
ipcMain.handle(IPC.scoringConfigSet, (_e, a: { project: string; patch: unknown }) => api.scoringConfigSet(a.project, a.patch));
...
ipcMain.handle(IPC.scoringPenalties, (_e, a: { project: string; includeVoided?: boolean }) => api.scoringPenalties(a.project, !!a?.includeVoided));
ipcMain.handle(IPC.scoringVoidPenalty, (_e, a: { id: string }) => api.scoringVoidPenalty(a.id));
ipcMain.handle(IPC.scoringScan, (_e, a: { project: string }) => api.scoringScan(a.project));
