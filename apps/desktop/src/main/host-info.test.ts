...
describe("parseProcesses", () => {
  it("parses `ps -eo pid=,pcpu=,pmem=,args=` (name from the command line) sorted by cpu", () => {
    const raw = [
      " 1234  92.4  8.1 node /srv/app/server.js --port 3000",
      "   42  12.0 22.5 postgres: background writer",
      " 9001 150.2  3.0 /opt/cc/cc --flag",
      "RCWSUM cores=8 mem=42",
    ].join("\n");
    // interpreter (node) → the script it runs; plain exe → its basename; summary line ignored.
    expect(parseProcesses(raw)).toEqual([
      { pid: 9001, name: "cc", cpu: 150.2, mem: 3.0 },
      { pid: 1234, name: "server.js", cpu: 92.4, mem: 8.1 },
  ...
  it("parses the `ps aux` fallback and skips its header row", () => {
    const raw = [
      ...
      "root  777 40.5  2.1 12345  6789 ?   Ssl  10:00 0:03 /usr/bin/node /app/index.js",
      "pg    888  5.0 30.0 99999 88888 ?   S    09:00 1:20 /usr/lib/postgres",
    ].join("\n");
    expect(parseProcesses(raw)).toEqual([
      { pid: 777, name: "index.js", cpu: 40.5, mem: 2.1 },
      { pid: 888, name: "postgres", cpu: 5.0, mem: 30.0 },
    ]);
  });

  it("caps at 12 and returns empty for garbage", () => {
    const many = Array.from({ length: 20 }, (_, i) => `${i + 1} ${i} 1.0 proc${i}`).join("\n");
