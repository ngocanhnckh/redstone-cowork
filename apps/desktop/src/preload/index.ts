...
contextBridge.exposeInMainWorld("cowork", {
  ...
  directMode: (): Promise<{ mode: string; choice: string | null; available: boolean }> => ipcRenderer.invoke(IPC.directMode),
  directModeSet: (mode: "cloud" | "direct" | null): Promise<{ mode: string }> => ipcRenderer.invoke(IPC.directModeSet, { mode }),
  directHosts: (): Promise<unknown[]> => ipcRenderer.invoke(IPC.directHosts),
  directHostAdd: (a: { label?: string; sshHost: string; user?: string | null; port?: number | null; opts?: string[] }): Promise<unknown> => ipcRenderer.invoke(IPC.directHostAdd, a),
  directHostRemove: (id: string): Promise<{ ok: boolean }> => ipcRenderer.invoke(IPC.directHostRemove, { id }),
  directHostEnable: (id: string, enabled: boolean): Promise<unknown> => ipcRenderer.invoke(IPC.directHostEnable, { id, enabled }),
  ...
  agencyMissionDetail: (key: string): Promise<import("../shared/agency").AgencyMissionDetail> => ipcRenderer.invoke(IPC.agencyMissionDetail, { key }),
  ...
  agencyAgent: (id: string): Promise<import("../shared/agency").AgencyAgentDossier> => ipcRenderer.invoke(IPC.agencyAgent, { id }),
  agencyGithubRoster: (): Promise<Array<{ accountId: string; found: boolean; contribTotal: number; activeDays: number; commits: number }>> => ipcRenderer.invoke(IPC.agencyGithubRoster),
  agencyWeek: (): Promise<import("../shared/agency").AgentOfWeek> => ipcRenderer.invoke(IPC.agencyWeek),
  agencyWeekConfig: (cfg: import("../shared/agency").WeekConfig): Promise<import("../shared/agency").WeekConfig> => ipcRenderer.invoke(IPC.agencyWeekConfig, cfg),
  scoringBoard: (project: string, week?: string): Promise<import("../shared/scoring").ScoringBoard> => ipcRenderer.invoke(IPC.scoringBoard, { project, week }),
  scoringMy: (project?: string): Promise<import("../shared/scoring").MyScore> => ipcRenderer.invoke(IPC.scoringMy, { project }),
  scoringHistory: (project?: string): Promise<import("../shared/scoring").ScoreHistory> => ipcRenderer.invoke(IPC.scoringHistory, { project }),
  scoringProjects: (): Promise<import("../shared/scoring").ScoringProjectRef[]> => ipcRenderer.invoke(IPC.scoringProjects),
  scoringAgentWeek: (): Promise<import("../shared/scoring").AgentWeekAward> => ipcRenderer.invoke(IPC.scoringAgentWeek),
  scoringConfigGet: (project: string): Promise<import("../shared/scoring").ScoringProjectConfig> => ipcRenderer.invoke(IPC.scoringConfigGet, { project }),
  scoringConfigSet: (project: string, patch: Partial<import("../shared/scoring").ScoringProjectConfig>): Promise<import("../shared/scoring").ScoringProjectConfig> => ipcRenderer.invoke(IPC.scoringConfigSet, { project, patch }),
  ...
  scoringPenalties: (project: string, includeVoided: boolean): Promise<import("../shared/scoring").ScoringPenaltyView[]> => ipcRenderer.invoke(IPC.scoringPenalties, { project, includeVoided }),
  scoringVoidPenalty: (id: string): Promise<{ ok: boolean }> => ipcRenderer.invoke(IPC.scoringVoidPenalty, { id }),
  scoringScan: (project: string): Promise<{ completions: number; penalties: number }> => ipcRenderer.invoke(IPC.scoringScan, { project }),
  ...
  injectAppCss: (webContentsId: number, css: string): Promise<{ ok: boolean }> =>
    ipcRenderer.invoke(IPC.appInjectCss, { webContentsId, css }),
